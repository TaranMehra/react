...
export const refreshController = async (req: Request, res: Response) => {
  ...
  jwt.verify(refreshToken, process.env.REFRESH_TOKEN_SECRET as string, async (err:any, decoded: any) => {
    if (err) {
      return res.status(403).json({ success: false, message: "Invalid or expired refresh token", expire: true});
    }
    ...
    console.log('User fetching accessToken through refreshToken');
